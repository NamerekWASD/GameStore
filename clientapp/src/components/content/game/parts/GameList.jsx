                                ...
                                <div key={game.id} className={"card rounded-0 w-100 bg-secondary bg-gradient" + cardClassName} style={{ cursor: 'pointer', height: '180px' }}
                                    ...
                                    <div className="no-gutters d-flex flex-row align-items-center gap-5">
                                        <div style={{ minWidth: "120px", width: "120px", maxWidth: "120px", height: '150px' }} className="overflow-hidden m-3">
                                            <img src={game.image ? game.image.path : game.images.find(item => item.type.name === POSTER).path} alt={game.title} className="width-inherit" />
                                        </div>
                        ...
                        games.map(game => {
                            return (
                                <Card key={game.id} className={"m-2 bg-light-gray rounded-0 overflow-hidden pointer " + cardClassName} onClick={() => navigateToDetails(game, navigate)} >
                                    ...
                                    src={game.image ? game.image.path : game.images.find(item => item.type.name === POSTER).path}
